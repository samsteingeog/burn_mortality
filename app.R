...
ui <- fluidPage (
  navbarPage(id="DS421", "DS421 Project",
             ...
             tabPanel("About",
                      ...
                      fluidRow(column(12, p("There has been an increase in wildfires in California over the past decade, 
                                 coinciding with the death of 129 million trees due to drought and bark beetles. 
                                 Wildfires cause varying degrees of damage to ecosystems, but the factors that influence burn severity in wildfires are complex."))),
                      fluidRow(column(12, 
                                      p("The goal of this project is to further explore the drivers of burn severity in California wildfires, including presence of dead biomass due to tree death. 
                             This study is based on a sample of 102 wildfires (>300 acres) that occurred in California in 2017. 
                             Our team worked with CalFire’s Fire Research and Assessment Program to review our methodology and identify data sources for environmental variables."))),
                      fluidRow(column(12, 
                                      p("For more information about land-owner assistance programs, visit:"))),
                      fluidRow(column(12,
                                      a("Cal Fire Land Owner Assistance Program", 
                                        href = "https://www.fire.ca.gov/programs/resource-management/resource-protection-improvement/landowner-assistance"))),
                      fluidRow(column(12,
                                      a("USDA Disaster Recovery Assistance Program", 
                                        href = "https://www.nrcs.usda.gov/wps/portal/nrcs/detail/national/programs/?cid=NRCSEPRD1361073"))),
                      
                      fluidRow(column(12, 
                                      p(" "))),
                      fluidRow(column(12, 
                                      p("This project was produced as part of the Data Science for the 21st Century program at UC Berkeley. 
                             Questions can be directed to Sam Stein at samstein@berkeley.edu. 
                             Our team consisted of Sam Hing, Sam Stein, Yiyi He, and Arfa Aijazi. 
                             Our team would like to thank Mark Rosenberg from CalFire for his invaluable assistance with this project."))),
                      fluidRow(column(12,
                      ...
                      #    fluidRow(column(4, h4("Summary of Fires"), p("We sampled 102 wildfires that occurred in California in 2017. 
                      #Here is the average distribution of average burn severities of the fires we selected."), plotOutput('piechart')))
             ),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$DS421, {
    if (input$DS421 == "Fire Data") {
      showModal(modalDialog("Click on a fire in the map on the right to see its burn severity, 
                          live biomass, dead biomass, tree density, land cover type, and weather parameters. 
  ...
  labelsDBR = c("Enhanced Regrowth, High", "Enhanced Regrowth, Low", "Unburned", 
                "Low Severity", "Modereat-Low Severity", "Moderate-High Severity", "High Severity")
  
  # Have maps appear when user clicks a fire
  observeEvent(input$map_marker_click, {
    ...
    output$treedensity_map <- renderLeaflet({
      leaflet() %>%
        addTiles %>%
        addProviderTiles("Esri.WorldImagery") %>%
        addRasterImage(treedensity_fire(), colors = paletteTD, opacity = 1, 
                       maxBytes = Inf, project = FALSE) %>%
        addLegend(position = "bottomleft",
                  pal = paletteTD,
                  values = domainTD,
                  opacity = 0.7,
    ...
    output$table <- renderTable({
      ave_data2 %>%
        ...
        dplyr::rename(
          ...
          "Prior week avg temp (F)" = week_temp, 
