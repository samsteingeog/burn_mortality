...
sum(abs(ave_ols1$residuals))
sum(abs(ave_ols2$residuals))
...
sum(abs(ave_AIC1$residuals))
sum(abs(ave_AIC2$residuals)) #second version with summed biomass performs slightly better
...
sum(abs(ave_svm_1$residuals)) #Check residuals; best for all models so far
