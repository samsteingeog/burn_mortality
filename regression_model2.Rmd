...
ave_ind_alt <- ave_df %>% #trying out summed biomass values (instead of mean)
  dplyr::select(slope_MEAN, aspect_MEAN, DEM_MEAN, LiveBio_SUM, DeadBio_SUM, fireduration, GIS_ACRES, 
          month_temp, week_temp, month_precip, week_precip, wind_speed, month_humid, week_humid)
...
#create the model (using all data for now)
#current model doesn't include precip/temp/humid during the fire or solar radiation data
ave_ols1 <- lm(dbr_means ~ slope_MEAN + aspect_MEAN + DEM_MEAN + LiveBio_MEAN + DeadBio_MEAN + fireduration + GIS_ACRES + 
          month_temp + week_temp + month_precip + week_precip + wind_speed + month_humid + week_humid, 
          data = ave_df)

ave_ols2 <- lm(dbr_means ~ slope_MEAN + aspect_MEAN + DEM_MEAN + LiveBio_SUM + DeadBio_SUM + fireduration + GIS_ACRES + 
          month_temp + week_temp + month_precip + week_precip + wind_speed + month_humid + week_humid, 
          data = ave_df)
...
which(is.na(all_ind$week_humid)) #lines 7596-7865 are NA
...
ggplot(ave_df) +  #using means seems better here
...
png(file = "graphics/SVMAve.png")

ggplot(ave_df) + 
  geom_point(aes(x = dbr_means, y = svm_dbr_alt), alpha = 0.4) +
  geom_abline(slope = 1) +
  xlim(0,1000) +
  ylim(0,1000) +
  theme_minimal() +
...
#slightly better than ols stepwise (I think?) see plot below
